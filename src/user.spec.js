...
describe('Auth module', () => {
  ...
  it('#isAuthenticated = false when there is a token but it expired', () => {
    ...
    const user = getUser(null, emptyHashWindow);
  ...
  describe('location has a hash', () => {
    ...
    const window = {
      location: {
        hash: '#access_token=FAKE&token_type=Bearer&expires_in=3600&state=reactApp',
      },
      history: {
        pushState: jest.fn(),
      },
    ...
    beforeAll(() => {
      ...
      const user = getUser(null, window);
  ...
  it('constructs an api object', () => {
    ...
    const user = getUser(ApiClass, emptyHashWindow);
    user.getApi();
